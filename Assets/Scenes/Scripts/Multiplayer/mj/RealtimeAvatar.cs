...
#pragma warning restore 0649
        ...
        private int myCount; // to assign avatars  to their correct positions
        ...
        void Start()
        {
            ...
            try
            {
                ...
                myCount = _realtimeAvatarManager.avatars.Count-1;  // to solve: when early person quit and rejoin, they would overlap late person
            }
        ...
        void UpdateAvatarTransformsForLocalPlayer()
        {
            ...
            // Root
            if (_localPlayer.startPoints != null)
            {
                /*// Only update position if it is owned by the local client.
                if (!realtimeView.isOwnedLocallySelf)
                    return;*/

                // move player to the defined position
                _localPlayer.player.position = _localPlayer.startPoints[myCount].position;
                // move the instantiated avatar to the defined position;
                transform.position = _localPlayer.startPoints[myCount].position;
                _localPlayer.player.rotation = _localPlayer.startPoints[myCount].rotation;
                transform.rotation = _localPlayer.startPoints[myCount].rotation;              
                transform.localScale = _localPlayer.startPoints[myCount].localScale;
            }
            ...
            if (_localPlayer.head != null)
            {
                ...
                _head.localPosition = _localPlayer.head.localPosition;
                _head.localRotation = _localPlayer.head.localRotation;
            }
            else
            {
            ...
            if (_leftHand != null)
            {
                if (_localPlayer.leftHand != null)
                {
                    ...
                    _leftHand.localPosition = _localPlayer.leftHand.localPosition;
                    _leftHand.localRotation = _localPlayer.leftHand.localRotation;
                }
                else
                {
            ...
            if (_rightHand != null)
            {
                if (_localPlayer.rightHand != null)
                {
                    ...
                    _rightHand.localPosition = _localPlayer.rightHand.localPosition;
                    _rightHand.localRotation = _localPlayer.rightHand.localRotation;
                }
                else
                {
